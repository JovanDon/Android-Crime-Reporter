	...
	if(isset($_GET['apicall'])){
		...
		switch($_GET['apicall']){
			...
			case 'uploadpic':
				...
				if(isset($_FILES['pic']['name']) && isset($_POST['type']) && isset($_POST['description'])){
					
					//uploading file and storing it to database as well 
					try{
						move_uploaded_file($_FILES['pic']['tmp_name'], UPLOAD_PATH . $_FILES['pic']['name']);
						$stmt = $conn->prepare("INSERT INTO cimes (type,description,imagelink) VALUES (?,?,?)");
						$stmt->bind_param("ss",$_POST['type'],$_POST['description'] $_FILES['pic']['name']);
						if($stmt->execute()){
